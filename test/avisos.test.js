...
test('solo avisa dentro de la ventana de su hora', () => {
  const a = (h, m) => avisos.tocaAvisar('20:30', new Date(2026, 6, 30, h, m));
  assert.strictEqual(a(20, 29), false, 'un minuto antes todavía no');
  assert.strictEqual(a(20, 30), true,  'en punto sí');
  assert.strictEqual(a(20, 44), true,  'dentro del margen sí');
  assert.strictEqual(a(20, 45), false, 'pasado el margen ya no');
  assert.strictEqual(a(9, 0),   false, 'por la mañana no');
});

test('una hora mal escrita no dispara avisos', () => {
  // Antes que avisar a deshora, no avisar.
  for (const mala of ['', null, 'luego', '25:00', '20-30', undefined]) {
    assert.strictEqual(avisos.tocaAvisar(mala, new Date(2026, 6, 30, 20, 30)), false, `"${mala}" no debería disparar`);
  }
});
